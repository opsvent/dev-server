		...
		return resp.view('/src/api/views/dashboard.ejs', {
			monitors: await Promise.all(
				ctx.jobsDef.jobs.map(async jobDef => {
					const statusByInstance = await Promise.all(
						Object.keys(ctx.clients).map(async instance => {
							...
							const events = await ctx.db.conn
								...
								.find({
									where: {
										monitorId: jobDef.id,
										reporterNode: instance,
										timestamp: MoreThanOrEqual(
											subDays(startOfDay(new Date()), 60)
										)
									},
									order: { timestamp: 'ASC' }
								});
							...
							const last60Days = _.times(60, i => {
								...
								const downtime = downtimeIntervals.reduce(
									(acc, cur) =>
										acc + getOverlappingSeconds(day, cur),
									0
								);

								const upPercent =
									Math.round(
										((86400 - downtime) / 86400) * 100
									) / 100;
							...
							return {
								instance,
								up: current?.toStatus == MonitorStatus.UP,
					...
					return {
						...jobDef,
						status: statusByInstance.every(
							instanceStatus => instanceStatus.up
						)
							? 'up'
							: statusByInstance.every(
									instanceStatus => !instanceStatus.up
							  )
							? 'down'
							: 'partial',
						byInstance: statusByInstance
					};
				})
			)
		});
